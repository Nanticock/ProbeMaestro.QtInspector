...
#include <QObjectViewer/RTTIHelper/rttiTraversalExample_msvc.cpp>
...
// all past releases of Qt are available for download at: https://download.qt.io/archive/qt/
// use this to build extensions
int main(int argc, char **argv)
{
    std::cout << "MSVC Runtime Version: " << getMSVCRuntimeVersion() << std::endl;

    // TODO: in order to maximize compatibility with different applications,
    // all the code in this module should be written using target platform API (WIN API, LINUX API, etc..)
    // and plain C with static c-runtime.
    // for more info on how to do this on windows:
    // https://stackoverflow.com/a/2766337/10780320
    ...
    loader.addInitializationFunction(initMemoryMaps);
    loader.setErrorHandler(ExecutableLoader::LoadingErrorHandler, loaderErrorHandler);
    loader.setErrorHandler(ExecutableLoader::RunningErrorHandler, runningErrorHandler);

    loader.runExecutableAsync("Application.modified.exe", argc, argv);

    while (!QGuiApplication::instance())
    {
        using namespace std::chrono_literals; // For using suffixes like s, ms, us for seconds, milliseconds and microseconds respectively
        std::this_thread::sleep_for(500ms);   // Sleep for 0.5 seconds
    }
